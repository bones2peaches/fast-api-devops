...
class UserActions {
  ...
  // Creates a new user
  create() {
    const url = `${this.protocol}://${this.domainName}/api/${this.stage}/user`;
  ...
  // Logs a user in
  login() {
    const url = `${this.protocol}://${this.domainName}/api/${this.stage}/user/session`;
    const payload = JSON.stringify({
      username: this.username,
      password: this.password,
    });
  ...
  logout() {
    ...
    const res = http.del(url, null, params); // Note: http.del(url, body, params) - body is set to null
